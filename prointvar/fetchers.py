    ...
    if not os.path.exists(outputfile.rstrip('.gz')) or override:
        if pdb:
            ...
            if bio:
                ...
                # original mmCIF?
                # url = config.pdbe_download + "{}.cif".format(pdbid)
                url = config.http_pdbe + ("entry-files/download/"
...
def download_alignment_from_cath(identifier, max_sequences=1000, override=False):
    """
    Downloads a MSA in fasta format from CATH to the filesystem.

    :param identifier: (str) CATH ID (<Superfamily>_<Funfam>)
    :param max_sequences: (str) Maximum number of sequences
    :param override: (boolean)
    :return: (side effects)
    """
...
def download_alignment_from_pfam(identifier, override=False):
    """
