...
if __name__ == '__main__':
    ...
    mmcif = d.atoms(add_res_full=True)
    # mmcif = get_mmcif_selected_from_table(mmcif, atom=('CA',))
    # print(mmcif.head())

    inputdssp = "{}{}{}.dssp".format(c.db_root, c.db_dssp_generated, pdbid)
    r = DSSPgenerator(inputcif, inputdssp).run()  # override=True)

    d = DSSPreader(inputdssp)
    dssp = d.residues(add_rsa_class=True, add_ss_reduced=True)
    # print(dssp.head())

    # extend the table with unbound RSAs
    dssp_runner_split_chains([pdbid], bio=True)  # , override=True)
    rsas_values = OrderedDict()
    for c1 in dssp.CHAIN.unique():
        ninputdssp = "{}{}{}_{}.dssp".format(c.db_root, c.db_dssp_generated, pdbid, c1)
        nd = DSSPreader(ninputdssp)
        nd = nd.read()
        rsas_values[c1] = list(nd.loc[:, 'RSA'])
    nrsas = []
    for c2 in dssp.CHAIN_FULL.unique():
        c1 = c2[0]
        nrsas += [v for v in rsas_values[c1]]
    assert len(nrsas) == len(dssp)
    dssp['RSA_UNB'] = nrsas
    from prointvar.dssp import add_dssp_rsa_class

    dssp = add_dssp_rsa_class(dssp, rsa_col='RSA_UNB')

    inputsifts = "{}{}{}.xml".format(c.db_root, c.db_sifts_xml, pdbid)
    d = SIFTSreader(inputsifts)
    sifts = d.read(add_regions=True, add_dbs=False)
    # print(sifts.head())
    # print(list(sifts))

    # t = table_merger(mmcif, dssp, None)
    # t = table_merger(mmcif, None, sifts)
    # t = table_merger(None, dssp, sifts)
    # t = table_merger(mmcif, dssp, sifts)
    # print(t.loc[0:10, :])
    # print(t[t.RSA != t.RSA_UNB].loc[:, ('CHAIN_FULL', 'RES',
    #                                     'RSA', 'RSA_CLASS', 'RSA_UNB', 'RSA_UNB_CLASS',)])
    # merged = t
    # assert len(mmcif) == len(merged)
    # print(len(mmcif))
    # print(len(t))

    t = TableMerger(mmcif, dssp, sifts)
    outputpickle = "{}{}{}_bio.pkl".format(c.db_root, c.tmp_dir_local, pdbid)
    t.merge(outputfile=outputpickle)
    t = TableMerger()
    t = t.load(inputfile=outputpickle)
    print(t.head())
    ...
    # uses all the mapping Pdb/Chains
    # for ix in data:
    #     uniprots.append(uniprot_id)
    #     pdbs.append(data[ix]['pdb_id'])
    #     chains.append(data[ix]['chain_id'])
    pass
