...
if __name__ == '__main__':
    ...
    # inputcif = "/Users/fabiomadeira/Downloads/{}-assembly-1.cif".format(pdbid)
    inputsifts = "{}{}{}.xml".format(c.db_root, c.db_sifts_xml, pdbid)

    d = SIFTSreader(inputsifts, verbose=False)
    d.residues(add_regions=True, add_dbs=False)
    # nd = d.to_json()
    # print(nd)
    # print(d.data.tail())
    # print(d.data.loc[:, [k for k in list(d.data) if k.startswith('PDB') or k.startswith('UniProt')]])

    # d = SIFTSreader(inputsifts, verbose=False)
    # d.residues(excluded=("InterPro", "GO", "EC", "NCBI", "PDB", "UniProt", "SCOP", "CATH"),
    #            add_regions=True, add_dbs=True)
    # d = d.to_json()
    # print(d)
    ...
    # print(d)
    # d = json.loads(d)
    # print(d["Pfam"]["dbSource"])
    t = d.data
    t = get_sifts_selected_from_table(t, chain_auth=('A',))
    # print(t.loc[:, ('PDB_entityId', 'PDB_dbChainId')])
    # print(t.PDB_entityId.unique())
    chaindict = OrderedDict()
    for ix in t.index:
        chain_auth = t.loc[ix, 'PDB_dbChainId']
        chain_label = t.loc[ix, 'PDB_entityId']
        if chain_auth not in chaindict:
            chaindict[chain_auth] = []
        if chain_label not in chaindict[chain_auth]:
            chaindict[chain_auth].append(chain_label)
    print(chaindict)
    pass
